...
- **체크섬 보존**: V8/V13 은 이미 적용된 마이그레이션이라 본문·주석을 **수정하지 않는다**. Flyway 체크섬은 주석까지 포함하므로, V8/V13 을 건드리면 그걸 이미 적용한 DB 의 부팅 validate(`Migration checksum mismatch`)가 깨진다(프로젝트 규칙: 적용된 마이그레이션 변경 금지). 누락분 해소 사실은 V22 헤더와 본 문서에만 기록한다.
