...
| `COUPON_SOLD_OUT` | 선착순 쿠폰 소진 (HTTP 409) *(계획)* |
| `COUPON_ALREADY_ISSUED` | 이미 발급받은 쿠폰 (HTTP 409) *(계획)* |
| `COUPON_MIN_ORDER_NOT_MET` | 최소 주문금액 미달 (HTTP 422) *(계획)* |
...
- 적용 대상 (예시):
  ...
  - `POST /coupons/{id}/issue`: 회원당 분당 10회 *(계획 — 선착순 폭주 시 1인 반복요청 억제)*
...
| POST | `/admin/coupons` | ADMIN | 쿠폰 정책 생성 *(계획)* |
| GET | `/admin/coupons` | ADMIN | 쿠폰 정책 목록 *(계획)* |
| PATCH | `/admin/coupons/{id}/status` | ADMIN | 쿠폰 상태 변경 (SUSPENDED/ENDED) *(계획)* |
| POST | `/admin/coupons/{id}/grant` | ADMIN | 특정 회원에게 직접지급 *(계획)* |
...
> 본 절은 **계획 단계** 명세다 — 구현(확장 W13) 시 본문이 확정된다. 설계 전문: [plans/coupon-system.md](./plans/coupon-system.md).
...
**Response 200**: `PageResponse<MemberCouponResponse>` — `MemberCouponResponse = { memberCouponId, couponId, name, discountType, discountValue, maxDiscountAmount, minOrderAmount, status, expiresAt, usedAt, orderNumber }`.
...
- **GET `/admin/coupons`** — 쿠폰 정책 목록. Query: `page`, `size`, `status` 선택. Response 200 `PageResponse<AdminCouponSummary>` (`{ couponId, name, discountType, discountValue, totalQuantity, issuedCount, status, validUntil }`).
...
| `CouponResponse` *(계획)* | couponId, name, discountType, discountValue, maxDiscountAmount, minOrderAmount, remainingQuantity, validUntil |
| `MemberCouponResponse` *(계획)* | memberCouponId, couponId, name, discountType, discountValue, maxDiscountAmount, minOrderAmount, status, expiresAt, usedAt, orderNumber |
