...
> **쿠폰 적용 (확장, *계획*)**: 회원 주문 Request 에 선택 필드 `"memberCouponId": 123` 을 둘 수 있다. 지정 시 ① 본인 보유·`ISSUED`·미만료 검증, ② `min_order_amount` 충족 검증(미달 시 422 `COUPON_MIN_ORDER_NOT_MET`), ③ 할인액 계산 후 `member_coupon` 을 USED 로 전이하고 주문에 연결한다. 응답에 `"discountAmount"`, `"payableAmount"`(= totalAmount − discountAmount) 가 추가된다. **결제는 `payableAmount` 를 청구**한다(§3.6). 게스트 주문에 `memberCouponId` 지정 시 422 `COUPON_NOT_APPLICABLE`. 추가 Errors: 404 `COUPON_NOT_FOUND` · 403/404 `COUPON_NOT_OWNED` · 422 `COUPON_EXPIRED` · 409 `COUPON_ALREADY_USED`.
...
| `OrderResponse` | orderNumber, status, totalAmount, discountAmount, payableAmount *(계획)*, items[], shipping, payment, createdAt |
