...
| 작성일 | 2026-05-05 |
| 최종 수정일 | 2026-06-30 |
| 변경 내용 | v2.5 — 멀티 인스턴스 분산 전환: 카탈로그 캐시 Caffeine→Redis 토글(ADR-17, #366), rate-limit 분산(Bucket4j-Lettuce, #367), 스케줄러 분산락(ShedLock, #365). §5.8·§11·§12 현행화. v2.4 — 토스페이먼츠 실 PG 연동(#321): dev/prod 프로파일 분리, `application-prod.yaml` 운영 하드닝, prod 약한 시크릿 거부. v2.3 — 탈퇴 회원 access 토큰 잔존을 stateless JWT 트레이드오프로 명시(§12 #6), 모든 쓰기 경로 서비스단 활성 회원 재검사 보강(#269). v2.2 — 카탈로그 read 캐시(Caffeine, ADR-14)·트랜잭셔널 아웃박스(ADR-15, #237)·클레임 도메인 반영. v2.1 — §12 알려진 한계와 트레이드오프 신설. v2.0 — 쿠폰 시스템(ADR-11)·Vue3 SPA(ADR-12)·Caffeine rate-limit(ADR-13) 반영, 패키지 구조 현행화. v1.x — 카탈로그·주문·회원 도메인 구현 반영. |
...
1. **단순성** — 단일 배포 단위(모듈러 모놀리스). 필요 시 수평 확장(§11.3)
...
- 마이크로서비스 분리
- 분산 트랜잭션·이벤트 소싱·CQRS
- 헥사고날/클린 아키텍처 풀 적용 (복잡도 대비 효용이 낮음)
...
| ADR-2 | 도메인 중심 패키지 구조 | 응집도, 변경 범위 국소화 | 레이어드 |
...
| ADR-11 | 동시성(쿠폰 발급): 원자적 조건부 UPDATE | 핫 카운터(`issued_count`)를 행 락 길게 잡지 않고 처리 — 비관적 락 대비 처리량 우위 ([decisions/coupon-concurrency.md](./decisions/coupon-concurrency.md)) | 비관적 락 / Redis 카운터 (조건부) |
| ADR-12 | 프론트엔드: Vue3 SPA + node-gradle 통합 빌드 | 단일 산출물(JAR)로 정적 서빙, `backend/`·`frontend/` 형제 구조 | 별도 배포 / 서버사이드 템플릿 |
| ADR-13 | Rate Limit 저장: Caffeine(in-memory) | 단일 인스턴스에서 외부 의존 없이 버킷 보관·만료 | Redis (멀티 인스턴스 시 전환) |
| ADR-14 | 카탈로그 read 캐시: Caffeine 로컬(M16 #236) → 멀티 인스턴스는 Redis 분산 전환(ADR-17, #366) | 앨범 상세/랜딩 목록 read 부하를 외부 의존 없이 흡수 — TTL 60s·`@CacheEvict` 무효화·`sync=true` 스탬피드 방지 | Redis 분산 캐시(다중화 시 → ADR-17) / 무캐시 |
...
- Redis — 카탈로그 분산 캐시(ADR-17, #366)·분산 rate-limit(#367)은 다중화에서 이미 도입. 남은 후보는 분산락(Redisson)·Refresh Token 이관(현재 DB 기반)
- Prometheus + Grafana
...
│   │     → 풀스캔·N+1 은 인덱스·페치 조인으로 해소. 인덱스 상세는 ERD §5.2.
│
├── cart/                       (장바구니) — 구현 완료
...
└── common/                     (횡단 관심사) — 구현 완료
    ...
    ├── scheduling/             (SchedulingConfig — @EnableScheduling + @EnableSchedulerLock(ShedLock, #365))
    ├── ratelimit/              (RateLimitFilter, RateLimitRegistry, RateLimitStoreConfig — Caffeine/Redis 토글 #367)
    ├── cache/                  (CacheConfig — @EnableCaching + CacheErrorHandler, 카탈로그 read 캐시 #236/분산 #366)
    └── outbox/                 (OutboxEvent(Publisher/Relay/Cleanup/Handler) — 트랜잭셔널 아웃박스, M16 #237)
```

> 도메인별 layout 은 `api / application / domain` 3-레이어로 통일하되, 횡단 보조(`security`, `exception`, `ratelimit`)가 필요한 도메인은 같은 레벨에 추가한다. 이전 controller/service/repository 명명에서 디렉토리 이름만 정리한 것으로 책임은 동일하다.
...
- **재고 차감 등 정합성 민감 영역**:
  - 비관적 락(`SELECT ... FOR UPDATE`)으로 동시 차감을 직렬화해 오버셀(lost-update)을 막는다.
  - 락이 없으면 오버셀이 발생하며, 이는 부하 테스트로 재현·측정했다(`docs/improvements/concurrency.md`).
  - 낙관적 락·Redis 분산락은 처리량 요구가 커지면 비교 대상이다.
...
한정수량 쿠폰의 `coupon.issued_count` 는 다수 트랜잭션이 동시에 갱신하는 **핫 카운터**다. 재고 오버셀과 동형 문제이며, 원자적 조건부 UPDATE 로 처리한다(ADR-11, [decisions/coupon-concurrency.md](./decisions/coupon-concurrency.md)).
...
### 5.8 카탈로그 read 캐시 (확장 M16 #236 → 분산 전환 #366)
앨범 조회 read 부하를 흡수하기 위해 캐시를 둔다. 저장소는 `spring.cache.type` 으로 토글한다 — **base/local 은 Caffeine 로컬(ADR-14), docker/prod 다중화는 Redis 분산(ADR-17, #366)**. Spring Cache 추상화(`common.cache.CacheConfig` 의 `@EnableCaching`)로 활성화하고, 캐시 이름·스펙은 `spring.cache` 자동구성으로 주입한다(별도 `CacheManager`/`RedisCacheConfiguration` 빈 없음 — 빈을 만들면 yaml 이 무시되므로 TTL·key-prefix 는 yaml 단일 소스).
...
- **스탬피드 방지**: 두 캐시 모두 `@Cacheable(sync = true)` — 키별 단일 로딩으로 동시 미스 시 thundering herd 차단. Redis 분산에서도 single-flight 는 노드 로컬이라 노드 간 동시 미스는 각자 한 번씩 로딩한다(표시용이라 무해).
- **트랜잭션 순서**: `@EnableCaching(order = LOWEST_PRECEDENCE − 1)` 로 캐시 advice 를 `@Transactional` 바깥에 둔다.
- **분산 전환(#366)**: Redis 모드는 `time-to-live: 60s`·key-prefix `groove:cache:v1:`(직렬화 스키마 버전), DTO(`AlbumDetailResponse`·`AlbumSummaryResponse`)는 `Serializable`+`serialVersionUID`(JDK 직렬화). `CacheConfig.CacheErrorHandler` 가 Redis 장애·역직렬화 실패를 **get→DB 폴백(미스 강등), evict/clear 실패는 전파**로 처리해 캐시가 가용성 단일점이 되지 않게 한다. 노드 간 무효화 일관성은 [decisions/catalog-cache-distributed.md](./decisions/catalog-cache-distributed.md) 참조.
...
설정 파일은 `backend/src/main/resources/application*.yaml` (base `application.yaml` + 프로파일별). 기본 경로(local/docker)는 Mock PG 이며 폴백 제거가 원칙이다. M17(실 PG 연동, #321)에서 토스 실 결제용 **dev/prod 프로파일**을 도입했다 — dev=토스 테스트 연동 확인, prod=실 결제 배포(§10.6 런북). `MockPaymentGateway`=`@Profile(local/test/docker)`, `TossPaymentGateway`/`TossPaymentConfig`=`@Profile(dev/prod)`로 결제 게이트웨이가 프로파일로 갈린다.

| Profile | 용도 | 특징 |
|---|---|---|
| local | 로컬 개발 | 로컬 MySQL, Mock PG 활성, 데모 시드 계정 |
| docker | Docker Compose | MySQL 컨테이너 연결, Mock PG 활성 (실 결제 아님) |
...
Rate Limit 버킷을 인스턴스 로컬 Caffeine 캐시에 두면(ADR-13, `common.ratelimit.RateLimitRegistry`), 로드밸런서 뒤에 N대를 두고 수평 확장할 때 인스턴스마다 독립 버킷을 유지해 **동일 IP/회원의 실효 한도가 N배**가 된다. 그 결과 로그인 무차별 대입·계정 열거(signup)·비밀번호 변경·쿠폰 사재기 억제력이 인스턴스 수에 비례해 약화된다.
...
각 확장 포인트에 대응하는 **현재 한계와 도입 판단 기준**은 §12에 정리했다.
...
| 카탈로그 캐시 멀티 인스턴스 일관성 필요 | ✅ **Redis 분산 캐시로 전환 완료(ADR-17, #366)** — `spring.cache.type` 토글(base=caffeine, docker/prod=redis), 노드 간 `@CacheEvict` 일관 |
| 한정반 동시성 강화 | `InventoryService`에 Redisson 분산락 |
...
수평 확장 토대(#375 — Redis·app 다중화·nginx 라운드로빈 LB)는 도입 완료. 노드 로컬 메커니즘은 분산 전환됨:
- Refresh Token: Redis 또는 DB 공유 (이미 DB 기반이라 영향 적음 — 미전환)
- Rate Limit: ✅ Bucket4j-Lettuce(Redis CAS) 공유 버킷(#367)
- 카탈로그 캐시: ✅ Redis 분산 캐시(#366)
- 스케줄러 중복 실행: ✅ ShedLock(MySQL `JdbcTemplateLockProvider`, #365) — 각 배치 노드 간 1회
...
아래 항목은 시스템의 알려진 한계와 그에 대한 설계 판단이다. §1의 우선순위(단순성 · 측정 가능성 · 조건부 확장)에 따라, 정합성(안전성)은 유지하면서 확장·운영 강화는 측정으로 필요가 확인될 때 도입한다. 각 한계는 코드 주석에 근거가 있고, 도입 트리거와 위치(§10.5·§11)를 함께 명시한다.

| # | 한계 | 근거 (코드) | 설계 판단 / 도입 트리거 |
|---|------|-------------|---------------------------|
...
| 2 | ~~스케줄러 **단일 인스턴스 가정** — 분산락 없음~~ → **해소(#365)** | `common.scheduling.SchedulingConfig`(`@EnableSchedulerLock`) + `@Scheduled` 배치 10종 + `V34__init_shedlock.sql` | **ShedLock**(`JdbcTemplateLockProvider(.usingDbTime())`, 새 인프라 0 — 기존 MySQL)로 각 배치를 노드 간 1회 실행 보장. 배치 메서드에 `@SchedulerLock`, 고빈도는 `lockAtLeastFor` 로 틱 겹침 차단 → §11.3. 설계는 [decisions/horizontal-scaling.md](./decisions/horizontal-scaling.md)(#274·#365) |
...
| 5 | ~~**분산 캐시·Redis 미도입** (로컬 캐시만)~~ → **해소(#366·#367)** | 카탈로그 read 캐시는 `spring.cache.type` 토글로 docker/prod=Redis 분산(ADR-17, #366 — `common/cache/CacheConfig`), rate-limit 도 Bucket4j-Lettuce Redis(#367). 멀티 인스턴스 토대 #375 | 단일 인스턴스(base/local)는 그대로 Caffeine — Redis 없이 데모 동작. 다중화(docker/prod)는 노드 간 무효화/한도 공유를 위해 Redis 분산. **분산락(Redisson)은 여전히 미도입** — 스케줄러는 ShedLock(MySQL, §12 #2), 한정반은 MySQL 비관락(#205)으로 충분 → **트리거·위치 §11.1** |
...
```
       ┌──────────────┐
       │ E2E (소량)   │  Bruno 컬렉션, 시드 후 수동/헤드리스 실행
       ├──────────────┤
...
- `@Sql`로 시드 데이터 주입
- **동시성 테스트**: `CountDownLatch` + `ExecutorService`로 동시 요청 시뮬레이션
...
- 시나리오:
  - `search.js` — 상품 검색
  - `order.js` — 주문 생성
  - `flash-sale.js` — 한정반 동시 주문 (오버셀 재현)
...
- ~~**시드 데이터 출처**~~: **결정됨** — 카탈로그는 Discogs 덤프, 트랜잭션은 Python Faker 하이브리드([decisions/seed-data.md](./decisions/seed-data.md)).
- ~~**한정반 동시성 Redis 도입 여부**~~: **종결** — 측정 결과 MySQL 비관락(#205)으로 oversell 0·목표 TPS 달성, Redis 분산락 비교(#210)는 한계 효용 낮아 컷(NOT_PLANNED). 쿠폰 핫 로우는 ADR-11 원자적 UPDATE 로 v1 해소. 상세는 §12 #5(분산 캐시·Redis 미도입 — 카탈로그 read 로컬 Caffeine 캐시는 #236 도입).
