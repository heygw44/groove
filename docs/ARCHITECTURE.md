...
| ADR-16 | 수평 확장 대비(스케줄러 분산락·rate-limit 분산): **유보 + 실행 설계**(#274) | 트리거(`replicas>1`) 미발생·멱등 안전망이 overlap 커버·#210 NOT_PLANNED 규율 일관 → 코드 없이 실행 설계만 기록([decisions/horizontal-scaling.md](./decisions/horizontal-scaling.md)) | 지금 ShedLock+Redis 도입 / rate-limit MySQL 분산 / 게이트웨이·WAF 이관 |
...
| 3 | rate-limit **인메모리** Caffeine — 단일 인스턴스 | `common/ratelimit/RateLimitRegistry.java` "수평 확장 제약(#164)" javadoc | 단일 인스턴스면 무해. 다중화 시 실효 한도 N배 → Bucket4j 분산 백엔드 또는 게이트웨이/WAF로 이관 → **상세 §10.5**, 위치 §11.1. 유보 결정·실행 설계는 [decisions/horizontal-scaling.md](./decisions/horizontal-scaling.md)(#274) |
