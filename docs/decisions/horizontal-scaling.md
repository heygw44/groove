...
### ② 스케줄러 분산락 = ShedLock (기존 MySQL, 새 인프라 0) — #365 로 구현 완료

> **실행됨(2026-06-30, #365).** 아래 설계대로 도입했다. 단 마이그레이션 번호는 그사이 V33 까지 진행돼
> `V31` 이 아닌 **`V34__init_shedlock.sql`** 로 작성했고(이 문서가 경고한 "설계-코드 드리프트"의 실제 사례),
> ShedLock 버전은 Spring Boot 4.x 호환 라인인 **7.7.0** 으로 고정했다. `@SchedulerLock` 타임아웃은
> `${groove.*}` 인라인 기본값 플레이스홀더로 외부화했다. 검증은 `docs/improvements/` 노트 참조.
