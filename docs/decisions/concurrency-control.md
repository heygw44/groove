...
전제도 분명하다. 인프라에 Redis 도 메시지 브로커도 없다(rate limit 은 Bucket4j 인메모리, 캐시는 Caffeine). [coupon-concurrency.md](./coupon-concurrency.md) 에서 이미 "새 인프라 없이 DB 로 푼다"는 방침을 세웠고, 운영은 단일 MySQL + 단일 앱 인스턴스를 가정한다. 재고 오버셀은 [overselling-baseline.md](../troubleshooting/overselling-baseline.md) 에 락 없는 Before 상태를 박제해 두고 비관적 락으로 개선하는 서사가 이미 깔려 있다.
...
`read → check → write` 를 아무 보호 없이 한다. lost-update 로 오버셀·초과발급이 그대로 난다. 운영에 쓸 수는 없고, [overselling-baseline.md](../troubleshooting/overselling-baseline.md) 의 Before 를 박제하는 용도로만 남겨 둔다.
