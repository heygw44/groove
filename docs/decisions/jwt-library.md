...
| 연관 이슈 | #9 |
| 작성자 | ParkGunWoo |
| 후속 작업 | JWT 발급/검증 모듈 구현 |

---

## Context

인증/회원 도메인에서 Access Token 과 Refresh Token 의 발급·검증·회전을 구현해야 한다. ARCHITECTURE.md ADR-5 에서 Stateless JWT 기반 인증을 채택했고, PRD/API 명세상 만족해야 할 요건은 다음과 같다.
...
**긍정적**
- HS256 + Access/Refresh 회전 구현이 짧은 코드로 끝난다.
