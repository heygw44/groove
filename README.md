...
- **추천** — 취향 프로필(장르·아티스트·연대)과 최근 본 상품·구매·위시리스트 같은 행동 신호를 가중치 합산해 홈에 "OO님을 위한 추천"으로 보여준다. 학습 모델 없이도 위시 홀드아웃 5-fold × 5시드(25회) 측정에서 recall@10 0.345 ± 0.060으로, 무작위 기준선(0.037) 대비 9.3배, 인기순 대조군(0.047) 대비 7.3배다
...
추천은 학습 모델 없이 가중치 합산 규칙으로 만들었고, 위시 홀드아웃을 5-fold × 5시드로 25회 반복 측정한 결과 recall@10 0.345 ± 0.060(측정 회원 30명·후보 상품 273건 기준, 무작위 기준선 0.037 대비 9.3배, 인기순 대조군 0.047 대비 7.3배)을 확인했다(`cd backend && ./gradlew precisionAt10`).
