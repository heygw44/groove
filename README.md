...
한정반 선착순 구매는 Redis Lua 스크립트로 중복 구매·재고 초과를 먼저 걸러내고, 살아남은 요청만 DB 트랜잭션(`SELECT ... FOR UPDATE` + unique 제약 + 조건부 재고 갱신)을 태우는 2단계 구조다. 로컬 1,000 VU 동시 요청에서 p95 802ms, 초과 판매 0건이었고, 운영 EC2 t3.micro로 도메인에 직접 부하를 걸었을 때는 1,000 VU에서 Nginx `worker_connections` 한계로 21.7%가 실패했지만 그 상황에서도 초과 판매는 0건이었다. 측정 방법과 원자료는 [`scripts/k6/results/limited-20260908.md`](scripts/k6/results/limited-20260908.md), [`limited-prod-20260909.md`](scripts/k6/results/limited-prod-20260909.md)에 있다. `worker_connections`를 4096으로 올리고 upstream keepalive를 붙인 뒤 재측정(2026-09-16)한 결과 1,000 VU 실패는 0건, 2,000 VU까지 올려도 실패율 1.55%로 초과 판매는 여전히 0건이었고, 다음 병목은 Tomcat accept-count(100)로 좁혀졌다. [`limited-prod-20260916.md`](scripts/k6/results/limited-prod-20260916.md) 참고. 이어서 접속 큐 세 겹(Nginx listen 백로그·컨테이너 SYN 큐·Tomcat accept-count)을 4096으로 맞춘 뒤에는 2,000 VU 실패도 0건이 되고 SYN 재전송 계단이 사라졌으며, 남은 지연은 1GB 호스트의 메모리 압박(스왑)과 DB 행 락 컨보이로 좁혀졌다. [`limited-prod-20260916-backlog.md`](scripts/k6/results/limited-prod-20260916-backlog.md) 참고. JVM 비힙·MySQL 메모리 상한을 걸어 평시 스왑은 946MB에서 334MB로 줄였지만 러시 순간의 스왑아웃은 1GB 호스트에서 해소되지 않았고(2,000 VU에서 504 실패 204건·Redis 서킷 OPEN), 인스턴스 업그레이드 판단은 #439로 넘겼다. [`limited-prod-20260916-memory.md`](scripts/k6/results/limited-prod-20260916-memory.md) 참고. 구매 진입을 메타 캐시로 바꿔 탈락 응답(409)이 재고 소진 전에 즉시 나가게 됐고, 남은 병목은 당첨자 100명의 DB 쓰기(커넥션 10개·스왑 iowait)로 좁혀졌다. [`limited-prod-20260916-purchase.md`](scripts/k6/results/limited-prod-20260916-purchase.md) 참고.
...
추천은 학습 모델 없이 가중치 합산 규칙으로 만들었고, 위시 홀드아웃을 5-fold × 5시드로 25회 반복 측정한 결과 recall@10 0.347 ± 0.058(측정 회원 30명·후보 상품 262건 기준, 무작위 기준선 0.038 대비 9.1배, 인기순 대조군 0.047 대비 7.4배)을 확인했다(`cd backend && ./gradlew precisionAt10`).
