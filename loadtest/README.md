...
- `catalog-cache.js` — 카탈로그 조회 캐시 Before/After (#236, M16): 상세(`/albums/{id}`)·랜딩(`/albums`) 핫경로를 ramping-vus 로 측정 → `detail_latency`/`landing_latency` p95 박제. teardown 이 `/actuator/metrics/cache.gets` 로 적중률 로깅(local 프로파일). 캐시 OFF(SPRING_CACHE_TYPE=none) vs ON 앱을 상대로 같은 스크립트 실행.
...
결과 JSON 은 `loadtest/flash-sale-summary.json`(최근 실행 = After, 1000 VU)에 보존되고, Before/After 박제본은
각각 `flash-sale-before-summary.json`(#205 락 임시 해제 = `placeWithoutLock` 경로, 1000 VU: created 224·
lost-update 124·finalStock 0)·`flash-sale-after-summary.json`(#205 비관락: created 100·lost-update 0)에 보존된다.
Before 는 `SPRING_CACHE_TYPE=none` 으로 띄운 동일 시드(앨범 50,000)에서 측정했다.
...
`Page<AlbumSummaryResponse>`(PageImpl) JDK 직렬화 왕복 성공을 박는다 — 단일 인스턴스 캐시 거동은
