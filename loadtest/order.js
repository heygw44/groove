// 주문 생성 부하 테스트 (#193) — 다중 상품 주문(POST /api/v1/orders)을 ramping-vus 로 측정한다.
//
// 시드 사용자 풀(loadtest001..080@groove.test / Test1234!)의 Bearer 토큰으로 매 iteration 마다 서로 다른
// 앨범 3종을 주문한다. 재고 차감은 락 없이 단순 구현된 초기 경로이며, 본 부하는 그
// 처리량/지연 Before 베이스라인을 측정하는 것이 목적이다 — 동시성 정합성은 별도 동시성 테스트에서 측정한다.
//
