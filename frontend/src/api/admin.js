...
// 관리자 콘솔 API — 전부 ADMIN 전용 `/api/v1/admin/**`. Bearer 는 client 인터셉터가 자동 첨부.

// ── 앨범 ───────────────────────────────────────────────────────────────────

/** 관리자 앨범 목록. GET /admin/albums → PageResponse<AlbumSummaryResponse>. HIDDEN 포함 전체 status 반환. */
...
/** 주문 상태 강제 전환. PATCH /admin/orders/{orderNumber}/status → AdminOrderResponse. 전이 규칙 위반 시 409. */
...
/** 주문 환불(취소). POST /admin/orders/{orderNumber}/refund → AdminRefundResponse(멱등). 재요청 시 alreadyRefunded=true. */
