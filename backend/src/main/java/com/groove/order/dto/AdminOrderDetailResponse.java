...
public record AdminOrderDetailResponse(
	...
	public static AdminOrderDetailResponse from(Order order) {
		...
		return new AdminOrderDetailResponse(order.getId(), order.getOrderNumber(), order.getMember().getId(),
				...
				order.getFinalAmount(), items, ShippingAddressResponse.from(order.getShippingAddress()),
				order.getCreatedAt(), order.getExpiresAt(), order.getCanceledAt(), order.getCancelReason());
	}
}
