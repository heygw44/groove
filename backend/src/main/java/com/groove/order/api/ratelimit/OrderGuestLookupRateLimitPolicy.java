 ...
 *
 * 게스트 조회는 비인증 공개 엔드포인트라 회원 키가 없다. orderNumber+email 페어 추측 공격은 한 IP 의 총 시도량을
 * 제한하는 것이 정석이므로(email 을 키에 넣으면 email 을 바꿔 우회 가능) login·signup 과 동일하게
 * RateLimitKeyResolver.clientIp() 로 IP 키잉한다. RateLimitFilter 가 Security 보다 먼저 실행돼도 IP 는 항상
 * 결정되므로 폴백이 불필요하다.
 *
 * 한도/리필 주기는 OrderRateLimitProperties 에서 주입받고, 초과 시 RateLimitFilter 가 429 + Retry-After 를 작성한다.
 */
@Component
