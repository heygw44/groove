...
public class AdminLimitedDropStatsService {
	...
	public List<LimitedDropStatsResponse> getLimitedDropStats() {
		return adminStatsMapper.findLimitedDropStats().stream()
				.map(row -> LimitedDropStatsResponse.of(row, resolveAttempts(row)))
				.toList();
	}
	...
	private LimitedDropAttemptStats resolveAttempts(LimitedDropStatsRow row) {
		...
		Map<LimitedAttemptResult, Long> attempts = limitedDropRedisService.getAttempts(row.dropId());
