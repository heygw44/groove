...
/**
 * 부분 수정 요청. 필드가 null 이면 기존 값을 유지한다. totalQuantity·maxDiscountAmount 는
 * 키 생략=유지, null=해제, 값=교체.
 */
public record CouponUpdateRequest(
		...
		JsonNullable<@DecimalMin(value = "0", inclusive = false, message = "최대 할인 한도는 0보다 커야 합니다.")
		@Digits(integer = 8, fraction = 2,
				message = "최대 할인 한도는 정수부 8자리, 소수부 2자리 이하여야 합니다.") BigDecimal> maxDiscountAmount,

		JsonNullable<@Positive(message = "총 수량은 1 이상이어야 합니다.") Integer> totalQuantity,
