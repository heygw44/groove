 ...
 *
 * <p>배송은 물리적 진행(스케줄러=택배사 시뮬레이션)을 반영해 항상 전진시키되, 주문이 종착 상태(환불 CANCELLED 등)라
 * 배송을 더 이상 따라가지 못하면 상태 발산을 {@code WARN} 으로 드러낸다 — 환불 시 배송 자체를 중단/취소하는 보강은
 * 별도 과제다(배송 상태 머신에 취소 상태 부재).
 */
@Service
public class ShippingService {
    ...
    /**
     * 배송 전이에 주문을 락스텝으로 따라가게 한다. {@link Order#advanceTo} 는 합법 전이만 수행하므로(아니면 무해 무시)
     * 정상 흐름·관리자 선전이(이미 도달)에는 영향이 없다. 다만 주문이 종착 상태(취소/결제실패)라 배송을 따라갈 수
     * 없으면 — 배송은 계속 전진하되 — 상태 발산을 로그로 드러낸다(조용히 어긋나지 않도록, CodeRabbit 리뷰 반영).
     */
