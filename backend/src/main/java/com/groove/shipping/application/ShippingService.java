 ...
 * 배송 조회 + 자동 진행 트랜잭션 경계.
 *
 * advanceToShipped/advanceToDelivered 는 자동 진행 스케줄러가 식별자로 호출하는 상태 전이 단위 트랜잭션 —
 * 배송이 기대 상태일 때만 전이하고 아니면 무해하게 무시한다. 전이 분기에서 같은 트랜잭션의 주문도 Order.advanceTo 로
 * 락스텝 전진시킨다(합법 전이만).
 *
 * 주문 행은 findByIdForUpdate(PESSIMISTIC_WRITE)로 잠그고 최신 상태를 재조회한다 — 같은 주문 행 락을 잡는
 * cancelPartially(반품 부분취소)와 직렬화된다. 관리자 환불(RefundSteps)은 결제 행만 잠가 주문 락을 공유하진 않지만,
 * 락 후 종착 재검증 + 주문 행 쓰기 직렬화 덕에 이미 CANCELLED 된 주문을 SHIPPED/DELIVERED 로 되살리는 lost update 는
 * 발생하지 않는다. 주문이 종착 상태면 자동 진행을 건너뛴다.
 */
@Service
public class ShippingService {
    ...
    /** 주문에 연결된 배송의 완료(DELIVERED) 시각 — 배송이 없거나 아직 DELIVERED 가 아니면 Optional.empty. */
    @Transactional(readOnly = true)
    ...
    /** 주문에 연결된 배송을 취소(CANCELLED)시킨다. 배송이 없거나 이미 종착(DELIVERED/CANCELLED)이면 무시한다. */
    @Transactional
    ...
    /**
     * 자동 진행 한 단계 — 주문을 Order.advanceTo 로 먼저 전진시키고 합법 전이로 성공했을 때만 배송도 같은
     * 트랜잭션에서 전이(mark)시킨다. 둘을 항상 락스텝으로 묶어 주문 PREPARING·배송 SHIPPED 같은 비대칭을 막는다.
     *
     * 주문 행은 findByIdForUpdate 로 잠그고 종착 여부를 재검증한다 — cancelPartially 와 직렬화되고, 발송 전 취소·환불이
     * CANCELLED 로 만든 주문을 자동진행이 덮어쓰지 않는다. order 프록시는 .getId() 외로 건드리지 않는다 —
     * findByIdForUpdate 로 신선하게 하이드레이션하기 전에 만지면 1차 캐시의 stale 상태를 읽는다.
     */
