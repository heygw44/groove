...
/**
 * 토스 successUrl/failUrl 콜백의 토큰이 결제에 저장된 callback_token 과 일치하지 않는 경우.
 *
 * <p><b>상태 코드:</b> 도메인 에러코드 분류는 403(PAYMENT_CALLBACK_TOKEN_MISMATCH)이지만, success 콜백 컨트롤러는
 * 이 예외를 흡수해 클라이언트에는 302 fail 리다이렉트로 응답한다(JSON 미노출). 403 은 GlobalExceptionHandler 가
 * 다른 경로에서 이 예외를 받았을 때의 분류값일 뿐, 실제 콜백 응답은 302 다.
 *
