...
/**
 * 결제 결과 웹훅 수신 API(permitAll). X-Mock-Signature 서명 검증(실패 시 401).
 * 멱등은 본문 pgTransactionId 키로 보장한다. 웹훅·폴링 중복 수신에도 전이는 1회.
 * Mock 프로파일 전용이다. {@link WebhookSignatureVerifier} 유일 구현이 @Profile(local/dev/test/docker)라 prod 엔 빈이 없다.
 * prod 실 결제 inbound 는 TossWebhookController 경로이므로 prod 에서 제외해 미충족 의존성 기동 실패를 막는다.
 */
