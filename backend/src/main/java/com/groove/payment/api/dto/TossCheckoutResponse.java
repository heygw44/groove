 ...
 * successUrl/failUrl 은 결제별 토큰(callback_token)이 쿼리에 박힌 서버 콜백 URL로, 위젯의 requestPayment 에 그대로 넘긴다.
...
public record TossCheckoutResponse(
        ...
        String successUrl,
        @Schema(description = "결제 실패 콜백 URL — 결제별 토큰 쿼리 포함. 위젯 requestPayment 의 failUrl 로 사용 (dev/prod 만)",
                example = "http://localhost:8080/payments/toss/fail?token=...")
        String failUrl) {
}
