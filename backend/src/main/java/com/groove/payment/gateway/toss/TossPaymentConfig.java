...
/**
 * 토스페이먼츠 연동 구성. dev/prod 에서만 활성(그 외는 MockPaymentGateway).
 * 커넥션 풀로 {@link JdkClientHttpRequestFactory}({@link HttpClient})를 쓴다. JDK21 HttpClient 는 keep-alive 를 풀링·재사용해
 * 매 호출 소켓을 새로 여는 SimpleClientHttpRequestFactory 와 달리 토스 지연 시 톰캣 워커가 소켓 생성에 묶이지 않는다.
 * connect 타임아웃은 HttpClient, read 타임아웃은 RequestFactory 에 건다.
 * status handler 를 두지 않는다. 토스 오류는 RestClientResponseException 으로 전파되고 호출부 래퍼가 502 로 일괄 래핑한다.
 * 여기서 직접 던지면 더블래핑되므로 호출부에 위임한다.
 */
...
public class TossPaymentConfig {
    ...
    /**
     * Authorization: Basic 헤더를 주입한다. 시크릿 키 뒤에 콜론을 붙여(빈 패스워드) UTF-8 base64 인코딩하는 토스 인증 규약.
     */
    ...
    /**
     * 토스 confirm/query/refund 공유 서킷브레이커. 일시 장애(5xx·연결 실패)만 실패로 집계하고 느린 호출 비율도 함께 본다.
     * 실패율/느린호출율이 임계를 넘으면 OPEN 되어 후속 호출을 빠르게 실패시켜 워커 점유를 끊는다.
     */
    @Bean
    ...
    /**
     * 토스 confirm/query/refund 공유 재시도. 일시 장애(5xx·연결 실패)만 재시도한다.
     * 동기 confirm 워커 점유·UX 보호로 시도 횟수·백오프를 작게 둔다(읽기 타임아웃은 재시도 안 함, 어댑터 술어 참고).
     */
    @Bean
