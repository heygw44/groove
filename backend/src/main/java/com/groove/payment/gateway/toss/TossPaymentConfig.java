...
public class TossPaymentConfig {
    ...
    public RestClient tossRestClient(TossPaymentProperties props) {
        // connect 타임아웃은 풀링 HttpClient 에, read 타임아웃은 RequestFactory 에 건다(#320).
    ...
    /**
     * 토스 confirm/query/refund 공유 서킷브레이커(#320). 일시 장애(5xx·연결 실패)만 실패로 집계하고,
    ...
    /**
     * 토스 confirm/query/refund 공유 재시도(#320). 일시 장애(5xx·연결 실패)만 재시도하며, 동기 confirm 워커 점유·UX 보호를 위해
