 ...
 * fail(failUrl)은 상태 무변경 안내만(미인증 콜백 CSRF 방어) — 보상은 폴링 리퍼가 만료 경로로 수행한다.
 * 토스는 confirm 모델이라 게이트웨이 request() 가 없어 PaymentService(request 경로)와 분리하고, 외부 confirm 은 트랜잭션 밖에서 호출한다.
 */
@Service
