 ...
 * fail(failUrl)은 상태를 바꾸지 않고 안내만 한다(미인증 콜백 CSRF 방어). 보상은 폴링 리퍼가 만료 경로로 수행한다.
 * 토스는 confirm 모델이라 request() 경로가 없어 PaymentService 와 분리하고, 외부 confirm 은 tx 밖에서 호출한다.
 */
@Service
public class TossPaymentService {
    ...
    // 요청 단계의 잠정 pgTransactionId 접두사. confirm 전엔 paymentKey 를 몰라 orderNumber 로 채운다.
    // 실제 paymentKey 와 값 공간이 겹치지 않게 접두사를 둔다(uk_payment_pg_tx 우연 충돌·폴링 오조회 방지).
    ...
    /** 결제 요청(checkout). payable 을 PENDING 으로 저장하고 결제위젯 초기화 값을 응답한다. 기존 결제는 멱등 응답. */
    public TossCheckoutResponse checkout(Long callerMemberId, PaymentCreateRequest request) {
        ...
        // 결제별 무작위 토큰을 successUrl 로만 round-trip 시켜 confirm 이 일치 검증한다(교차 주문 조작 차단).
        ...
        // persistPending 이 동시 충돌(uk_payment_order)로 기존 결제를 복원했으면 우리 token 은 저장되지 않았다.
        // 응답 successUrl 에는 항상 "실제 저장된" 토큰을 쓴다.
        PendingPayment pending = persistPending(prep, request, token);
        return buildResponse(pending.response(), pending.token());
    }

    /** props 부재(mock)면 URL 은 null. */
    private TossCheckoutResponse buildResponse(PaymentApiResponse persisted, String token) {
        ...
        // 토큰은 confirm 이 검증하는 successUrl 에만 싣는다. failUrl 은 상태 무변경이라 불필요하다(노출면 축소).
    ...
    /**
     * successUrl confirm 처리. 저장 payable 과 리다이렉트 amount 일치를 confirm 전에 검증해 위변조를 막고,
     * paymentKey 기준 멱등 래퍼로 confirm + PAID 적용을 1회만 수행한다(successUrl 재호출 안전).
     */
    public PaymentCallbackResult confirm(String paymentKey, String orderId, long amount, String token) {
        ...
        // 토큰 검증을 최우선으로 둬 미인증 콜백의 교차 주문 조작을 차단한다. 이후 단계는 모두 이 관문 뒤에서만.
    ...
    /** token 은 그 결제에 "실제 저장된" 콜백 토큰(충돌 복원 시 우리 token 과 다를 수 있다). */
    private record PendingPayment(PaymentApiResponse response, String token) {
    }

    /** 토스는 request() 호출 없이 잠정 pgTx 로 PENDING Payment(+콜백 토큰)를 저장한다. */
    private PendingPayment persistPending(PaymentRequestPrep prep, PaymentCreateRequest request, String callbackToken) {
        ...
        try {
            ...
            // uk_payment_order 동시 충돌. 승자가 저장한 기존 결제와 토큰을 재조회한다(우리 token 은 저장되지 않았다).
    ...
    /**
     * confirm(외부 호출, tx 밖) 후 PAID 면 적용. 비-PAID(가상계좌 PENDING 등)는 잠정 pgTx 를 실제 paymentKey 로 교체해
     * 후속 웹훅/폴링이 정산하게 한 뒤, 멱등 캐시에 종착 결과를 남기지 않도록 예외로 빠져나간다(재호출 시 재처리 허용).
     */
    ...
    /**
     * 저장 토큰과 successUrl 로 들어온 token 을 상수시간 비교한다(타이밍 노출 최소화).
     * 저장 토큰 null(레거시 결제)·수신 token null/불일치는 모두 거부한다.
     */
