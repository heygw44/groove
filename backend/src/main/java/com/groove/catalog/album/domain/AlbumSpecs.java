...
/**
 * 앨범 공개 검색용 Specification 모음 (#34, API §3.3 GET /albums).
 *
 * 각 메서드는 단일 필터를 표현하며 입력이 null/blank 일 때 conjunction()(noop)을 반환해 동적 조합 시 영향이 없도록 한다.
 *
 * keyword 는 비정규화된 artist_name 을 포함한 단일 테이블 FULLTEXT(title, artist_name)를 MATCH ... AGAINST(... IN
 * BOOLEAN MODE)로 검색한다 (#204) — 풀스캔을 fulltext 인덱스 접근으로 전환한다. artist/genre/label 동반 페치는
 * AlbumRepository 의 @EntityGraph(#203)가 담당하며 Specs 자체는 fetch 하지 않는다.
 */
public final class AlbumSpecs {
    ...
    /**
     * label 필터. label 은 nullable FK 라 root.get("label") 의 암묵 inner join 으로 label 미지정 앨범은 자연히 제외된다 —
     * 특정 labelId 로 거를 때 의도된 동작. genre 와 동일하게 fetch 는 하지 않아 의도된 N+1(W10 시연)을 보존한다.
     */
    ...
    /**
     * 키워드 검색: 비정규화 단일 테이블 FULLTEXT(title, artist_name)를 MATCH(title, artist_name) AGAINST(? IN BOOLEAN
     * MODE)로 검색한다 (#204). relevance score > 0 인 행을 매칭으로 본다.
     *
     * 왜 비정규화 + FULLTEXT 인가: 기존 LOWER(title) LIKE '%k%' OR LOWER(artist.name) LIKE '%k%' 는 선행 와일드카드 +
     * LOWER() 래핑으로 인덱스를 못 타 50,000건 풀스캔이었다(베이스라인 #196). title 과 artist.name 이 서로 다른 테이블이라
     * 단순히 두 FULLTEXT 를 OR 로 묶으면 cross-table OR 가 인덱스를 무력화하므로, artist 이름을 album 에 비정규화해 단일
     * FULLTEXT 한 방으로 구동한다 → type=ALL → fulltext.
     *
     * 의미 차이: ngram(token=2) 파서 + 따옴표 구문(phrase)으로 부분일치(substring) 의미를 보존하지만, 1글자 키워드는 ngram
     * 최소 토큰(2) 미만이라 매칭되지 않는다. 사용자 입력의 boolean-mode 연산자(+ - > < ( ) ~ * " @)는 제거해 의도치 않은 FT
     * 문법 해석을 막는다.
     */
    ...
    /**
     * FULLTEXT boolean-mode 연산자/구분자를 제거한 뒤 따옴표 구문(phrase)으로 감싼다 — ngram 파서가 연속된
     * bigram(부분일치)으로 매칭하도록. 연산자만으로 이뤄진 입력은 빈 문자열을 반환해 호출 측이 noop 으로 처리한다. 닫는
     * 따옴표가 구문을 깨지 않도록 입력의 " 도 함께 제거한다.
     */
