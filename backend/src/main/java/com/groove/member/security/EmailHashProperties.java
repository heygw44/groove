 ...
 *
 * application.yaml 의 email.hash.* 키와 1:1 매핑된다. JWT 서명키(JwtProperties) 와 의도적으로 분리한다
 * — 키 1개=용도 1개(key separation, NIST SP 800-57). JWT 시크릿 롤링이 이메일 점유 해시를 깨뜨리면 안
 * 되기 때문이다. compact constructor 에서 검증하므로 잘못된 운영 설정은 빈 생성 시점에 즉시 실패한다.
 * secret 은 HMAC-SHA256 키이므로 최소 32바이트(256bit) 이상이어야 하며, .env.example 의 플레이스홀더
 * 값이면 SecretPlaceholderGuard 가 기동을 거부한다(이슈 #165).
 */
