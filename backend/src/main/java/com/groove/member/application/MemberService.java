...
public class MemberService {
    ...
    /**
     * 탈퇴를 차단하는 "진행 중" 주문 상태. 결제됐고 아직 배송이 끝나지 않은 상태들 —
     * 미결제(PENDING)와 종착 상태(DELIVERED/COMPLETED/CANCELLED/PAYMENT_FAILED)는 제외한다.
     */
     ...
     * 회원가입.
     *
     * 이메일 중복(soft delete·익명화 포함) 검사 → BCrypt 해시 → 영속화. 중복 검사는 평문이 아닌 정규화
     * 이메일 해시(EmailHasher)로 한다 (#170 패턴 A, #186) — 탈퇴 익명화가 평문을 치환해도 email_hash 는
     * 보존되므로 같은 이메일의 재가입이 차단된다. 신규 회원은 v1(HMAC) 해시로 저장하되, 점유 검사는 v1 과
     * legacy(SHA-256) 를 함께 보아 마이그레이션 이전 탈퇴 회원(평문 파기로 재계산 불가)도 차단한다.
     *
     * 선체크(existsByEmailHashIn)와 save 사이에 동시 가입 요청이 끼어들 수 있다. DB UNIQUE
     * 제약(uk_member_email_hash)이 최종 방어선이며, 위반 시 DataIntegrityViolationException 을 도메인
     * 예외로 변환해 409 응답이 보장된다.
     *
     ...
     *
     * name·phone 만 변경하며, 미전송(null) 필드는 유지된다 (Member.updateProfile). 더티 체킹으로
     * 트랜잭션 커밋 시 반영되므로 별도 save 호출은 불필요하다.
     *
     ...
     *
     * 본인 비밀번호를 BCrypt 비교로 재확인한 뒤, "진행 중" 주문이 없으면 deleted_at 을 찍고 회원 PII 를
     * 익명화(Member.anonymize — email 평문 치환·name/phone 마스킹, email_hash 는 보존)한 다음
     * MemberWithdrawnEvent 를 발행한다. 이벤트의 AFTER_COMMIT 리스너가 장바구니 삭제(cart)와 리프레시
     * 토큰 정리(auth)를 각 모듈에서 수행한다 — 본 서비스는 자기 도메인(member soft delete·익명화)만
     * 책임지고, 타 모듈 데이터는 직접 건드리지 않는다. 주문/배송 PII 는 배송완료 후 보존기간이 지나면
     * OrderPiiAnonymizationScheduler 가 별도로 익명화한다 (#170 Part B).
     *
     * 멱등: access 토큰은 stateless 라 탈퇴 직후에도 만료 전까지 유효하므로 재요청이 들어올 수 있다.
     * 활성/탈퇴 회원을 모두 조회(findById)해 이미 탈퇴 상태면 no-op 으로 끝낸다 — 이벤트를 다시 발행하지
     * 않고 주문 검사도 생략한다. 따라서 반복 호출은 항상 204 로 수렴한다. findById 를 쓰는 이유는 getMyInfo·
     * updateMyInfo 가 findByIdAndDeletedAtIsNull(활성 한정, 탈퇴 시 404)인 것과 달리, 탈퇴는 멱등이어야
     * 하므로 탈퇴자도 조회해 no-op 분기로 보내야 하기 때문이다.
     *
     * @throws MemberNotFoundException        해당 id 의 회원이 아예 없음 (404)
     * @throws AuthException                  비밀번호 불일치 (ErrorCode.MEMBER_PASSWORD_MISMATCH, 400)
