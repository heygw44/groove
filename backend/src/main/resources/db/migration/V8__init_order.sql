...
--
-- #42 범위. 본 이슈는 도메인 모델 + 상태 머신만 다룬다 — 주문 생성 API,
-- 재고 차감, orderNumber 발급기는 후속 이슈에서 도입한다.
--
-- 테이블명 'orders' 는 SQL 예약어 'order' 회피용 복수형 (ERD §3.0 표기 규칙).
--
-- 초기 인덱스 (UNIQUE + FK 기본만):
...
--
-- 후속 추가 예정 (슬로우 쿼리 측정 후, 본 V8 에서는 미포함):
...
--   - member_id XOR guest_email     : APP (OrderService 생성 시점) — DB 제약 미적용
