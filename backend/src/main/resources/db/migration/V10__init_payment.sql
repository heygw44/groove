...
--
-- #55 범위. 본 이슈는 결제 요청 접수(PENDING) + 상태 조회까지 — PAID/FAILED 전이와
-- paid_at / failure_reason 기록은 후속 웹훅/폴링에서 처리한다.
--
-- 멱등성 레코드 테이블(idempotency_record)은 ERD §4.12 이며 V9 에서 이미 도입했다.
--
-- 초기 인덱스 (UNIQUE + 웹훅 검증만):
--   - uk_payment_order   UNIQUE (order_id)            -- 주문당 결제 1건
--   - idx_payment_pg_tx  (pg_transaction_id)          -- PG 웹훅 검증용
--
-- 후속 추가 예정 (슬로우 쿼리 측정 후, 본 V10 에서는 미포함):
...
--   - paid_at: PAID 전환 시 기록                        : APP (후속 웹훅/폴링)
