	...
	class GetDetail {
		...
		void returnsDetailWithoutAuthentication() throws Exception {
			...
			AlbumDetailResponse response = new AlbumDetailResponse(5L, "Kind Of Blue",
					new ProductDetailResponse.ArtistSummary(1L, "Miles Davis"), 1959, "설명", List.of(pressing));
			given(albumService.getDetail(eq(5L))).willReturn(response);
		...
		void returnsNotFoundWhenAlbumMissing() throws Exception {
			// given
			given(albumService.getDetail(eq(99L))).willThrow(new BusinessException(ErrorCode.ALBUM_NOT_FOUND));
