...
class AlertConfigTest {
	...
	class AlertNotifierBean {
		...
		@Test
		@DisplayName("웹훅 URL 이 있으면 DiscordWebhookAlertNotifier 를 만든다")
		void createsDiscordNotifierWhenUrlPresent() {
			// given
			AlertProperties properties = new AlertProperties("https://discord.com/api/webhooks/test",
			...
			// then
			assertThat(notifier).isInstanceOf(DiscordWebhookAlertNotifier.class);
		}
	}
}
